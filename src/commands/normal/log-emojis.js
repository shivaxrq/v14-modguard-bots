...
const allah = require("../../../src/config");
const { istatistik } = require("../../../src/emoji.json");
...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (message.guild === null) {
        ...
        } else if (!allah.owner.includes(message.author.id)) {
            return message.reply({ content: ":x: Bot geliştiricisi olmadığın için kurulumu yapamazsın.", ephemeral: true });
        } else {
                ...
                .addComponents(
                    new StringSelectMenuBuilder()
                        ...
                        .addOptions([
                            {
                                label: `Kanal Kurulum`,
                                description: `Log Kanallarını Kurulum Sağlarsın`,
                                emoji: istatistik,
                                value: "kanal",
                            },
                            {
            ...
            let papaz = new EmbedBuilder()
                .setDescription(`**Kurulum Menüsü Aşağıdaki Menülerden Seçim Yapabilirsiniz. Lütfen** \`60 sn\` **seçim yapın, aksi takdirde işlem iptal edilecektir.**`)
